...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from("flip_posts").select("*").order("created_at", { ascending: false }),
  ...
  const avgDaysListed =
    soldFlips.length > 0
      ? Math.round(
          soldFlips.reduce(
            (s, f) => s + Math.floor((Date.now() - new Date(f.created_at).getTime()) / 86400000),
            0
          ) / soldFlips.length
        )
      : null;
  ...
  const filteredFlips =
    activeTab === "all" ? flips : flips.filter((f) => f.status === activeTab);
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* ── Status Summary ─────────────────────────────────── */}
            <div className={"mb-6 grid gap-2 " + (avgDaysListed !== null ? "grid-cols-4" : "grid-cols-3")}>
              ...
              {avgDaysListed !== null && (
                <div className="rounded-2xl border border-purple-500/20 bg-purple-500/10 p-3 text-center">
                  <div className="text-2xl font-black text-purple-400">{avgDaysListed}d</div>
            ...
            <section className="mb-8">
              <div className="mb-3 flex items-center justify-between">
                ...
                <Link href="/admin" className="text-xs font-black text-purple-400 underline">
                  + Add Flip
                </Link>
              </div>
              ...
              {filteredFlips.length === 0 ? (
                <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] py-12 text-center">
                  <p className="text-sm text-white/30">No flips yet.</p>
                  <Link
                    href="/admin"
                    className="mt-4 inline-flex rounded-2xl bg-purple-600 px-6 py-2.5 text-xs font-black uppercase tracking-[0.1em] text-white"
                  >
                    Add your first flip
                  </Link>
                </div>
                ...
                <div className="space-y-2">
                  {filteredFlips.map((flip) => {
                    const sc =
                      statusConfig[flip.status ?? ""] ?? {
                        color: "text-white/40",
                        label: flip.status ?? "Unknown",
                      };
                    const profit =
                      flip.status === "sold" &&
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-white/40">
                              ...
                              {flip.status === "sold" && (
                                <span>Sold {formatMoney(flip.sell_price)}</span>
                              )}
                              {profit !== null && (
                                <span
                                  className={
                              ...
                              <span>{daysAgo(flip.created_at)} ago · {formatDate(flip.created_at)}</span>
                            </div>
                          </div>
                          <div className="flex shrink-0 flex-col items-end gap-1.5">
                            <span className={"text-xs font-black uppercase " + sc.color}>
                              {sc.label}
                            </span>
                            <Link
                              href={`/admin/edit/${flip.id}`}
                              className="text-[10px] text-white/25 underline hover:text-white/50"
                            >
                              Edit
                            </Link>
                          </div>
            ...
            <section>
              <div className="mb-3 flex items-center justify-between">
                <h2 className="text-sm font-black uppercase tracking-tight text-white/70">
                  Scan History
                </h2>
                <Link href="/scan" className="text-xs font-black text-purple-400 underline">
                  New Scan
                </Link>
              </div>
                  ...
                  <div className="mb-3 grid grid-cols-3 gap-2">
                    <div className="rounded-2xl border border-green-500/20 bg-green-500/10 p-3 text-center">
                      <div className="text-xl font-black text-green-400">
                        {scans.filter((s) => s.verdict === "BUY").length}
                      </div>
                      <div className="text-[10px] font-black uppercase tracking-wide text-green-300/60">
                        Yes
                      </div>
                    </div>
                    <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-3 text-center">
                      <div className="text-xl font-black text-yellow-400">
                        {scans.filter((s) => s.verdict === "MAYBE").length}
                      </div>
                      <div className="text-[10px] font-black uppercase tracking-wide text-yellow-300/60">
                        Maybe
                      </div>
                    </div>
                    <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-3 text-center">
                      <div className="text-xl font-black text-red-400">
                        {scans.filter((s) => s.verdict === "SKIP").length}
                      </div>
                      <div className="text-[10px] font-black uppercase tracking-wide text-red-300/60">
                        Hell No
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    {scans.map((scan) => {
                      ...
                      return (
                        <div
                          key={scan.id}
                          className={"rounded-2xl border p-4 " + vc.border + " " + vc.bg}
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              <p className="truncate font-black text-white">
                              ...
                              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-white/40">
                                ...
                                <span
                                  className={
                                    scan.estimated_profit > 0 ? "text-purple-300" : ""
                                  }
                                >
                            ...
                            <div className={"shrink-0 text-sm font-black " + vc.text}>
                              {vc.label}
                            </div>
                          </div>
                        </div>
    ...
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
      ...
      <p
        className={
          ...
          (negative ? "text-red-400" : highlight ? "text-purple-300" : "text-white")
        }
      >
        {value}
      </p>
